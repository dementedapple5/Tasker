...
Call<List<Residencia>> req = service.getFavResis(moh.getUserName());
        req.enqueue(new Callback<List<Residencias»() {
@Override
public void onResponse(Call<List<Residencias» call, Response<List<Residencias» response) {
// Pasa lista del historial de compras al metodo para popular actividad
        lista2 = response.body();
        ResisFavAdapter adapter = new ResisFavAdapter(getContext(), lista2);
        rv.setAdapter(adapter);
        }

@Override
public void onFailure(Call<List<Residencias» call, Throwable t) {
